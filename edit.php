...
if( isset($_GET['status']) ) {
	...
	elseif ($_GET['status'] == "IDexists"){
		$StatusMessage = $language['statusMessageErrorIdExists'];
		$outputTop .= "<div class=\"message_status\">" . $StatusMessage . "</div>";
	}
	elseif ($_GET['status'] == "InputWrongFormat"){
		$StatusMessage = $language['statusMessageErrorInputWrongFormat'];
		$outputTop .= "<div class=\"message_status\">" . $StatusMessage . "</div>";
	}
	elseif ($_GET['status'] == "InputEmpty"){
		$StatusMessage = $language['statusMessageErrorInputEmpty'];
		$outputTop .= "<div class=\"message_status\">" . $StatusMessage . "</div>";
	}
	else {
...
for ($row = 0; $row < count($reminders); $row++) {
	...
	$output .= "<tr>";
	$output .= "<td>" .$ReminderTitle. "</td>"; //Titel
	$output .= "<td>".$ReminderID." (".$ReminderGroup.")</td>"; //ID
	
	if ($ReminderShiftable) {
		$output .= "<td>" .$ReminderInterval. "<br><span>".$language['tableShiftable']."</span></td>"; //Intervall
	}
	else {
		$output .= "<td>" .$ReminderInterval. "<br><span>".$language['tableNonShiftable']."</span></td>"; //Intervall
	}	
...
<span><?php echo $language['formLabelIdHint']; ?></span>
...
<label><?php echo $language['formLabelShiftableTitle']; ?></label>
<select name="ReminderShiftable">
  <option value="true"><?php echo $language['formLabelYes']; ?></option>
  <option value="false"><?php echo $language['formLabelNo']; ?></option>
</select>
<span><?php echo $language['formLabelShiftableHint']; ?></span>
